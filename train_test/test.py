...
def test_epoch(args, model, data_loader, loss_func):
	"""
	Evaluates the model on a test dataset.

	Returns:
		A tuple containing the maximum, minimum, average, and the spread (3*std) of relative errors.
	"""

	# Prevent gradient computations for efficiency
	with torch.no_grad():
		# Initialize list to store sum of relative errors for each item in each batch
		batch_errors = []
		print("Total sequence number", len(data_loader))
		...
		for batch in (data_loader):
			...
			for t in range(inputs.size(1)):
				...
				error = torch.sqrt(loss_func(HDM_approx, HDM) / loss_func(HDM_approx*0, HDM)) 
				batch_errors.append(error.cpu().numpy())
		batch_errors = np.asarray(batch_errors)
		return batch_errors.max(), batch_errors.min(),batch_errors.mean(),batch_errors.std()*3
